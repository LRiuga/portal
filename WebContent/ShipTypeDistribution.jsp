...
if(session!=null&&session.getAttribute("login")!=null){
	if(session.getAttribute("login").equals("ok")){
		...
		if(ftime==null||ftime==""){
			...
			{
				int result = 0;
				out.print("<h3>横坐标为船厂等级,纵坐标为船的等级</h3>");
				...
				Map<Integer,Map<Integer,Integer>> levelMap = ShipTypeDistribution.getTypeLevelAmountMap(fTime);
				...
				for(int level:levelSet){
					...
					for(int index=1;index<24;index++){
						out.print("<td>");
						if(levelMap.get(level).containsKey(index)){
							out.print(levelMap.get(level).get(index));					
						}else{
							out.print("");
						}
						out.print("</td>");
