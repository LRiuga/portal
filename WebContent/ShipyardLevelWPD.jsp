...
<%@page import="userStats.ShipyardLevelWPD"%>
...
<body>
	<h1>Each Shipyardlevel - Warehouse|Pet|DefenseSystem</h1>
	<br />
...
function cmGetY (obj){var y = 0;do{y += obj.offsetTop;obj = obj.offsetParent;}while(obj);return y;}
	...
	if(item.equals("Warehouse")){
		type = "Warehouse";
	}else if(item.equals("Pet")){
		type = "Pet";
	}else if(item.equals("DefenseSystem")){
		type = "DefenseSystem";
	}
%>
			<select name="selectItem" id="selectItem">
				<option
					<%if(item.equals("Warehouse")){out.print("selected=\"selected\"");} %>
					value="Warehouse">Warehouse</option>
				<option
					<%if(item.equals("Pet")){out.print("selected=\"selected\"");} %>
					value="Pet">Pet</option>
				<option
					<%if(item.equals("DefenseSystem")){out.print("selected=\"selected\"");} %>
					value="DefenseSystem">Defense System</option>
	...
	if(session.getAttribute("login").equals("ok")){
		...
		if(ftime==null||ftime==""){
			...
			out.print("<table>");
			{
				...
				Map<Double,Double> maxMap = ShipyardLevelWPD.getMaxMap(item,fTime);
				Map<Double,Double> midMap = ShipyardLevelWPD.getMidMap(item,fTime);
				Map<Double,Double> avgMap = ShipyardLevelWPD.getAvgMap(item,fTime);
				
				for(double level=1;level<21;level++){
					out.print("<tr >");
