        ...
        chart = new Highcharts.Chart({
	     ...
	     //       {
	                <% 
	                	...
	                	itemList = DownloadStats.getItemList(fTime,tTime,item);
	                	...
	                	Map<Date,Double> map = DownloadStats.getOneKeyMap(fTime,tTime,item,itemList.get(i));
	                	out.print("data: [");
	                	StringBuffer sb = new StringBuffer();
	                	...
	                	map.put("KPI","KPI");
	                	map.put("OS","访问站点的手机操作系统");
	                	map.put("IsMatched","访问站点的手机是否匹配");
	                	map.put("IsWap","是否通过Wap访问");
	                	map.put("Brand","下载的手机型号");
	                	map.put("Model","访问站点的手机型号");
	                	map.put("Download","下载文件次数");
	                %>	   
     //       	}
            	]
        });
	...
	<div class=\"table\">
		<form action="" method="post">
			...
			<select name="selectItem" id="selectItem">
				<option
					<%if(item.equals("KPI")){out.print("selected=\"selected\"");} %>
					value="KPI">UV</option>
				<option
					<%if(item.equals("OS")){out.print("selected=\"selected\"");} %>
					value="OS">访问站点的手机操作系统</option>
				<option
					<%if(item.equals("IsMatched")){out.print("selected=\"selected\"");} %>
					value="IsMatched">访问站点的手机是否匹配</option>
				<option
					<%if(item.equals("IsWap")){out.print("selected=\"selected\"");} %>
					value="IsWap">是否通过Wap访问</option>
			</select>
			<h1><%=map.get(item)%>统计
			</h1>
			<h3>可选</h3>
...
if(itemList!=null&&itemList.size()>0){
	...
	out.print("日期");
	out.print("</th>");
	if(item.equals("KPI")||item.equals("IsMatched")){
		out.print("<th>");
		out.print("Rate");
		out.print("</th>");	
	}else{
	...
	for(int i=0;i<dateList.size();i++){
		oneDateMap = DownloadStats.getOneDateMap(dateList.get(i),item);
		Set<String> tempSet = oneDateMap.keySet();
		double total = 0.0;
		for(String temp:tempSet){
			total += oneDateMap.get(temp);
		}
		
		double rate = 0;
		if(item.equals("KPI")){
			if(oneDateMap.containsKey("UV")&&oneDateMap.get("UV")!=0){
				rate = oneDateMap.get("downloadUV")/oneDateMap.get("UV");
			}
		}else if(item.equals("IsMatched")){
			if(oneDateMap.containsKey("Success")&&oneDateMap.get("Success")!=0){
				if(oneDateMap.containsKey("Fail")){
					rate = oneDateMap.get("Success")/(oneDateMap.get("Success")+oneDateMap.get("Fail"));	
				}else{
					rate = 1;
				}
			}
		...
		for(String temp:itemList){
			...
			if(oneDateMap.containsKey(temp)){
				out.print(df.format(oneDateMap.get(temp)));			
			}else{
				out.print("");
			}
			out.print("</td>");
...
%>
	<%
out.print("<br>	");
out.print("1. KPI：主要指标。目前有UV");
out.print("<br>	");
out.print("2. 访问站点的手机操作系统：当天访问下载站点的手机所使用的操作系统");
out.print("<br>	");
out.print("3. 访问站点的手机是否匹配：当天访问下载站点的手机是否已被我们匹配。不匹配包括（1）没有取得手机型号（2）获得手机型号，但是没有被支持");
out.print("<br>	");
out.print("<br>	");
%>
</body>
