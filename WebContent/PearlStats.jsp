...
function cmGetY (obj){var y = 0;do{y += obj.offsetTop;obj = obj.offsetParent;}while(obj);return y;}
	...
	long j, k;
	{
		Map<String,Set<String>> typeMap = PearlStats.getAllItem(fromTime,toTime);
		...
		for (; j >= flag; j--) {
			...
			out.print("</td>");
			Map<String,Map<String,Long>> dataMap = PearlStats.getOneDayData(fTime);
			long allAmount = PearlStats.getGoldAmount("all",fTime);
			long inAmount = PearlStats.getGoldAmount("in",fTime);
			long outAmount = PearlStats.getGoldAmount("out",fTime);
			{
				out.print("<td>");
				if(allAmount>0){
					out.print(allAmount);	
				...
				for(String inType:inSet){
					...
					if(dataMap.containsKey("in")){
						if(dataMap.get("in").containsKey(inType)){
							out.print(PearlStats.Output(dataMap.get("in").get(inType)*1.0/inAmount));
						}else{
							out.print("");
						}
					}else{
				...
				for(String outType:outSet){
					...
					if(dataMap.containsKey("out")){
						if(dataMap.get("out").containsKey(outType)){
							out.print(PearlStats.Output(dataMap.get("out").get(outType)*1.0/outAmount));
						}else{
							out.print("");
						}
					}else{
