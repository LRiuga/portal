...
<%@ page
	import="java.util.*,OAStat.*,java.util.Date,java.text.*,actionStat.*,util.*"%>
...
%>
			<select name="selectItem" id="selectItem">
				<option
					<%if(item.equals("ShipyardStartfishing")){out.print("selected=\"selected\"");} %>
					value="ShipyardStartfishing">Each ShipyardLevel
					StartFishing</option>
				<option
					<%if(item.equals("ShipyardAttackFisher")){out.print("selected=\"selected\"");} %>
					value="ShipyardAttackFisher">Each ShipyardLevel
					AttackFisher</option>
				<option
					<%if(item.equals("PetTryAttackMonster")){out.print("selected=\"selected\"");} %>
					value="PetTryAttackMonster">Each PetLevel AttackMonster</option>
				<option
					<%if(item.equals("PetFinishAttackMonster")){out.print("selected=\"selected\"");} %>
					value="PetFinishAttackMonster">Each PetLevel KillMonster</option>
				<option
					<%if(item.equals("PetTryMonster")){out.print("selected=\"selected\"");} %>
	...
	if(session.getAttribute("login").equals("ok")){
		...
		if(ftime==null||ftime==""){
			...
			out.print("<table>");
			{
				...
				Map<Integer,Map<Integer,Integer>> levelMap = DAULevelAction.getLevelMap(table,type,fTime);
				Map<Integer,Integer> totalMap = new HashMap<Integer,Integer>();
				Map<Integer,Integer> maxMap = new HashMap<Integer,Integer>();
				Map<Integer,Double> midMap = new HashMap<Integer,Double>();
				Map<Integer,Integer> minMap = new HashMap<Integer,Integer>();
				
				int total = 0;
				Map<Integer,Integer> allUserMap = new HashMap<Integer,Integer>();
				...
				for(int level:levelSet){
					...
					for(int monetid:monetidSet){
						if(totalMap.containsKey(level)){
							totalMap.put(level,totalMap.get(level)+monetidMap.get(monetid));
						}else{
							totalMap.put(level,monetidMap.get(monetid));
						}
						list.add(monetidMap.get(monetid));
						
						total += monetidMap.get(monetid);
						if(allUserMap.containsKey(monetid)){
							allUserMap.put(monetid,allUserMap.get(monetid)+monetidMap.get(monetid));
						}else{	
							allUserMap.put(monetid,monetidMap.get(monetid));
						}
					}
					Collections.sort(list);
					maxMap.put(level,Integer.parseInt(MyUtil.getMax(list).toString()));
					midMap.put(level,Double.parseDouble(MyUtil.getMid(list).toString()));
					minMap.put(level,Integer.parseInt(MyUtil.getMin(list).toString()));
				}
				
				//处理
				ArrayList<Integer> tempList = new ArrayList<Integer>();
				Set<Integer> monetidSet = allUserMap.keySet();
				for(int monetid:monetidSet){
					tempList.add(allUserMap.get(monetid));
				}
				Collections.sort(tempList);
				totalMap.put(0,total);
				levelMap.put(0,allUserMap);
				maxMap.put(0,Integer.parseInt(MyUtil.getMax(tempList).toString()));
				midMap.put(0,Double.parseDouble(MyUtil.getMid(tempList).toString()));
				minMap.put(0,Integer.parseInt(MyUtil.getMin(tempList).toString()));
				...
				for(int level=0;level<21;level++){
					...
					if(levelMap.containsKey(level)){
						out.print(levelMap.get(level).size());					
					}else{
						out.print("");
					}
					out.print("</td>");
					...
					out.print("<td>");
					if(levelMap.containsKey(level)&&totalMap.containsKey(level)){
						out.print(df.format(totalMap.get(level)*1.0/levelMap.get(level).size()));					
					}else{
						out.print("");
					}
					out.print("</td>");
