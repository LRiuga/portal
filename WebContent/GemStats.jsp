...
java.awt.Font,
java.util.*,OAStat.*,java.util.Date,java.text.*,userStats.*,toolStat.GemStats"%>
	...
	<div class=\"table\">
		<form action="" method="post">
			...
			<br /> 过去一个月：<input name="time3" type="text" id="time3"
				...
				<option
					<%if(item.equals("1")){out.print("selected=\"selected\"");} %>
					value="1">Ruby</option>
				<option
					<%if(item.equals("2")){out.print("selected=\"selected\"");} %>
					value="2">Sapphire</option>
				<option
					<%if(item.equals("3")){out.print("selected=\"selected\"");} %>
					value="3">topaz</option>
				<option
					<%if(item.equals("4")){out.print("selected=\"selected\"");} %>
					value="4">emerald</option>
				<option
					<%if(item.equals("5")){out.print("selected=\"selected\"");} %>
					value="5">amethyst</option>
...
if(session!=null&&session.getAttribute("login")!=null){
	if(session.getAttribute("login").equals("ok")){
		...
		out.print("<table>");
			...
			long j, k;
			int gem = Integer.parseInt(item);
			//道具销售量统计表
			{
				Map<String,Set<String>> typeMap = GemStats.getAllItem(fromTime,toTime,gem);
				...
				for (; j >= flag; j--) {
					...
					out.print("</td>");
					Map<String,Map<String,Long>> dataMap = GemStats.getOneDayData(fTime,gem);
					long allAmount = GemStats.getGemAmount("all",fTime,gem);
					long inAmount = GemStats.getGemAmount("in",fTime,gem);
					long outAmount = GemStats.getGemAmount("out",fTime,gem);
					{
						out.print("<td>");
						out.print(allAmount);
						out.print("</td>");
						...
						for(String inType:inSet){
							...
							if(dataMap.containsKey("in")){
								if(dataMap.get("in").containsKey(inType)){
									out.print(GemStats.Output(dataMap.get("in").get(inType)*1.0/inAmount));
								}else{
									out.print("");
								}
							}else{
						...
						for(String outType:outSet){
							...
							if(dataMap.containsKey("out")){
								if(dataMap.get("out").containsKey(outType)){
									out.print(GemStats.Output(dataMap.get("out").get(outType)*1.0/outAmount));
								}else{
									out.print("");
								}
							}else{
