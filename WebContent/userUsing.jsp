...
<body>
 ...
 %>
	<h1><%=enType.getString("subtitle.items_used") %>:
	</h1>
	<a href="userUsing.jsp?Action=DownloadExcel"><img
...
if(session!=null&&session.getAttribute("login")!=null){
	if(session.getAttribute("login").equals("ok")){
		...
		if(action!=null&&action.equals("DownloadExcel")){
			response.setHeader("Content-disposition", "attachment; filename=ItemUsing.xls");
		}
		ArrayList<String> shopItemList = OAStatistic.getShopItemUsing();
		out.print("<table>");
		...
		itemName = OAStatUtil.getItemNameListUsingFrom167(fTime,tTime);
		...
		
		{
			...
			for (; j >= flag; j--) {
				...
				int total = 0;
				for (int i=0;i<shopItemList.size();i++) {
					if(itemUsed.get(shopItemList.get(i)) == null || itemUsed.get(shopItemList.get(i)) == 0){
					}else{
						total += itemUsed.get(shopItemList.get(i));
					}	
				}
				out.print("<td>");
				...
				
				{
					for (int i=0;i<shopItemList.size();i++) {
						...
						if(itemUsed.get(shopItemList.get(i)) == null || itemUsed.get(shopItemList.get(i)) == 0){
							...
							out.print("<a href = \"UsingDetail.jsp?itemName=" + shopItemList.get(i) + "&" + "fromTime=" + OAStatUtil.DateConvert(fromTime) + "&" + "type=all" + "\">" + itemUsed.get(shopItemList.get(i)) + "</a>" );
						}
						out.print("</td>");
					}
				}
