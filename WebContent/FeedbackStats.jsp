        ...
        chart = new Highcharts.Chart({
	     ...
	     //       {
	                <% 
	                	...
	                	map.put("LoginTime-Value","登录时间");
	                	map.put("ResourceFile-LoadingTime","资源列表读取时间");
	                	map.put("UpdateResourceFile-LoadingTime","资源更新时间");
	                	map.put("UpdateResourceFile-ResourceNumber","资源更新数量");
	                	map.put("UpdateResourceFile-ResourceSize","资源更新大小");
	                %>	   
     //       	}
            	]
        });
	...
	<div class=\"table\">
		<form action="" method="post">
			...
			<select name="selectItem" id="selectItem">
				<option
					<%if(item.equals("LoginTime-Value")){out.print("selected=\"selected\"");} %>
					value="LoginTime-Value">登录时间</option>
				<option
					<%if(item.equals("ResourceFile-LoadingTime")){out.print("selected=\"selected\"");} %>
					value="ResourceFile-LoadingTime">资源列表读取时间</option>
				<option
					<%if(item.equals("UpdateResourceFile-LoadingTime")){out.print("selected=\"selected\"");} %>
					value="UpdateResourceFile-LoadingTime">资源更新时间</option>
				<option
					<%if(item.equals("UpdateResourceFile-ResourceNumber")){out.print("selected=\"selected\"");} %>
					value="UpdateResourceFile-ResourceNumber">资源更新数量</option>
				<option
					<%if(item.equals("UpdateResourceFile-ResourceSize")){out.print("selected=\"selected\"");} %>
					value="UpdateResourceFile-ResourceSize">资源更新大小</option>
			</select>
			<h1><%=map.get(item)%>统计
			</h1>
			<h3>可选</h3>
...
out.print("1. 登录时间：");
out.print("<br>	");
out.print("2. 资源列表读取时间：获取更新列表时间");
out.print("<br>	");
out.print("3. 资源更新时间：实际更新时间");
out.print("<br>	");
out.print("4. 资源更新数量：实际更新资源数量");
out.print("<br>	");
out.print("5. 资源更新大小：实际更新资源大小");
out.print("<br>	");
out.print("<br>	");
%>
</body>
