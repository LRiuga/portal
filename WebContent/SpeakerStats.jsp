		...
		<form action="" method="post">
			...
			<br /> 过去一个月：<input name="time3" type="text" id="time3"
				...
				<option
					...
					value="1">SmallSpeaker</option>
				<option
					<%if(item.equals("2")){out.print("selected=\"selected\"");} %>
					value="2">BigSpeaker</option>
...
if(session!=null&&session.getAttribute("login")!=null){
	if(session.getAttribute("login").equals("ok")){
		...
		out.print("<table>");
			...
			long j, k;
			int speaker = Integer.parseInt(item);
			//道具销售量统计表
			{
				Map<String,Set<String>> typeMap = SpeakerStats.getAllItem(fromTime,toTime,speaker);
				...
				for (; j >= flag; j--) {
					...
					out.print("</td>");
					Map<String,Map<String,Long>> dataMap = SpeakerStats.getOneDayData(fTime,speaker);
//					long allAmount = SpeakerStats.getSpeakerAmount("all",fTime,gem);
					Map<String,Integer> map = Stocks.getStocks(fTime);
					long allAmount = 0;
					if(speaker==1){
						allAmount = map.get("SmallSpeaker");
					}else if(speaker==2){
						allAmount = map.get("BigSpeaker");
					}
					long inAmount = SpeakerStats.getSpeakerAmount("in",fTime,speaker);
					long outAmount = SpeakerStats.getSpeakerAmount("out",fTime,speaker);
					{
						out.print("<td>");
						out.print(allAmount);
						out.print("</td>");
						...
						for(String inType:inSet){
							...
							if(dataMap.containsKey("in")){
								if(dataMap.get("in").containsKey(inType)){
									out.print(SpeakerStats.Output(dataMap.get("in").get(inType)*1.0/inAmount));
								}else{
									out.print("");
								}
							}else{
						...
						for(String outType:outSet){
							...
							if(dataMap.containsKey("out")){
								if(dataMap.get("out").containsKey(outType)){
									out.print(SpeakerStats.Output(dataMap.get("out").get(outType)*1.0/outAmount));
								}else{
									out.print("");
								}
							}else{
