<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="gb2312"%>
<%@ page
	import="java.util.*,OAStat.*,java.util.Date,java.text.*,userStats.*,tribeStats.*"%>
...
if(fTime.compareTo(tTime)<0 && (tTime.getTime()-(new Date()).getTime())/1000/60/60/24 < 0){
			...
			//打印
			{
				...
				ArrayList<String> itemList = TribeTotal.getItemList();
					...
					{	
						for (int i = 0; i < itemList.size(); i++) {	
							if(i == itemList.size()-1){
								...
								out.print(TribeTotal.getItemAmount(itemList.get(i),fromTime));
								out.print("</td>");
								out.print("<td colspan=1 class=\"last\">");
								out.print(TribeTotal.getUsersAmount(itemList.get(i),fromTime));
								out.print("</td>");
							}else{
								out.print("<td colspan=1>");
								out.print(TribeTotal.getItemAmount(itemList.get(i),fromTime));
