	...
	if(session.getAttribute("login").equals("ok")){
		...
		String type = request.getParameter("selectItem");
		{
			...
			if((fTime.getTime()-(new Date()).getTime())/1000/60/60/24 < 0){
				ArrayList<String> list = ItemSource.getItemList(fTime,tTime);
				...
				for (; j >= flag; j--) {
					...
					java.util.Map<String,Integer> map = ItemSource.getItemMap(fromTime);
					...
					//直接读取一行数据
					{
						for (int i=0;i<list.size();i++) {
							out.print("<td colspan=1>");
							if(map.get(list.get(i)) == null || map.get(list.get(i)) == 0){
								out.print(" ");
							}else{
