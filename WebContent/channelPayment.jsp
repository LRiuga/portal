...
<%
	...
	thList.add("billing_stc");
	thList.add("billing_fortumo");
	thList.add("billing_payPal");
	thList.add("billing_googleWallet");
	thList.add("billing_Egypt");
	...
	titleMap.put("billing_fortumo","Fortumo(SAR)");
	titleMap.put("billing_Egypt","Egypt(EGP)");
	titleMap.put("billing_googleWallet","Google wallet(USD)");
	titleMap.put("billing_payPal","Paypal(SAR)");
	titleMap.put("billing_stc","Stc(SAR)");
	
	titleMap.put("user","用户数");
	titleMap.put("amount","金额");
	
	
	DBResultSet ds=DBUtil.db.execSQL("select * from userPayment where pdate >= ? and pdate <= ? and oakey like 'billing_%' order by pdate desc", new Object[]{startTime,endTime});
