...
<body>
	<h1>Each Shipyard Level ReturnRate</h1>
	<br />
...
if(session!=null&&session.getAttribute("login")!=null){
	if(session.getAttribute("login").equals("ok")){
		...
		if(ftime==null||ftime==""){
			...
			{
				int result = 0;
				out.print("<h3>横坐标为N天停留率,纵坐标为船厂等级</h3>");
				...
				Map<Integer,Map<Integer,Double>> levelMap = LevelReturnRate.getReturnRateMap(fTime);
				...
				for(int level:levelSet){
					...
					for(int index=1;index<31;index++){
						...
						if(levelMap.get(level).containsKey(index)){
							out.print(df.format(levelMap.get(level).get(index)));					
						}else{
							out.print("");
						}
						out.print("</td>");
