			...
			if(ftime==null||ftime==""){
				...
				//打印UV、PV
				{
					...
					for (; j >= flag; j--) {
						...
						//直接读取一行数据
						{	
							itemUVPV = OAStats_local.getUVPV_newUser(fromTime,toTime);
							userUVPV = OAStats_local.getUVPV_users_new(fromTime,toTime);
							
							for (int i = 0; i < newPages.size(); i++) {	
								if(i == newPages.size()-1){
									out.print("<td colspan=1 class=\"last\">");
									if(itemUVPV.get(newPages.get(i))==null){
										out.print(0);
									}else{
									...
									out.print("</td>");
									out.print("<td colspan=1 class=\"last\">");
									if(userUVPV.get(newPages.get(i))==null){
										out.print(0);
									}else{
