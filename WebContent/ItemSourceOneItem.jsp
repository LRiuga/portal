        ...
        chart = new Highcharts.Chart({
	     ...
	     //       {
	                <% 
	                	...
	                	itemList = ItemSource.getItemList(fTime,tTime,item);
	                	...
	                	Map<Date,Double> map = ItemSource.getOneKeyMap(fTime,tTime,item,itemList.get(i));
	                	out.print("data: [");
	                	StringBuffer sb = new StringBuffer();
	                	...
	                	map.put("Abomb","Abomb");
	                	map.put("BigBomb","BigBomb");
	                	map.put("SuperToolbox","SuperToolbox");
	                	map.put("LuckyCrew","LuckyCrew");
	                %>	   
     //       	}
            	]
        });
	...
	<div class=\"table\">
		<form action="" method="post">
			...
			<select name="selectItem" id="selectItem">
				<option
					<%if(item.equals("Abomb")){out.print("selected=\"selected\"");} %>
					value="Abomb">Abomb</option>
				<option
					<%if(item.equals("BigBomb")){out.print("selected=\"selected\"");} %>
					value="BigBomb">BigBomb</option>
				<option
					<%if(item.equals("SuperToolbox")){out.print("selected=\"selected\"");} %>
					value="SuperToolbox">SuperToolbox</option>
				<option
					<%if(item.equals("LuckyCrew")){out.print("selected=\"selected\"");} %>
					value="LuckyCrew">LuckyCrew</option>
				<option
					<%if(item.equals("SmallMissile")){out.print("selected=\"selected\"");} %>
					value="SmallMissile">SmallMissile</option>
				<option
					<%if(item.equals("BigMissile")){out.print("selected=\"selected\"");} %>
					value="BigMissile">BigMissile</option>
				<option
					<%if(item.equals("SmallToolbox")){out.print("selected=\"selected\"");} %>
					value="SmallToolbox">SmallToolbox</option>
				<option
					<%if(item.equals("BigToolbox")){out.print("selected=\"selected\"");} %>
					value="BigToolbox">BigToolbox</option>
			</select>
			<h1><%=map.get(item)%>统计
			</h1>
			<h3>可选</h3>
...
if(itemList!=null&&itemList.size()>0){
	...
	for(int i=0;i<dateList.size();i++){
		oneDateMap = ItemSource.getOneDateMap(dateList.get(i),item);
