	...
	<div class=\"table\">
		<form action="" method="post">
			<select name="selectItem" id="selectItem">
				<option
					<%if(item.equals("Download")){out.print("selected=\"selected\"");} %>
					value="Download">Download File Count</option>
			</select>
			<%=enType.getString("date") %>：<input name="time" type="text"
				id="time" onfocus="datelist.dfd(this)" readOnly="readonly">
			-<input name="time2" type="text" id="time2"
				onfocus="datelist.dfd(this)" readOnly="readonly"> <input
				name="yes" type="submit" value=<%=enType.getString("search") %> />
		</form>
	</div>
	<%
...
if(itemList!=null&&itemList.size()>0){
	...
	Map<String,Double> oneDateSuccessMap = new HashMap<String,Double>();
	...
	for(int i=0;i<dateList.size();i++){
		...
		double bb = 0;
		double j2me = 0;
		Set<String> phoneSet = oneDateMap.keySet();
		for(String phone:phoneSet){
			if(phone.contains("android")){
				android += oneDateMap.get(phone);
			}else if(phone.contains("bb")){
				bb += oneDateMap.get(phone);
			}else if(phone.contains("j2me")){
				j2me += oneDateMap.get(phone);
			}
		}
		oneDateSuccessMap = DownloadStats.getOneDateMap(dateList.get(i),"DownloadSuccessCount");
		...
		tempSet = oneDateSuccessMap.keySet();
		for(String temp:tempSet){
			totalSuccess += oneDateSuccessMap.get(temp);
		}
		...
		if(total==0){
			...
			out.print(df2.format(totalSuccess/total));
		}
		out.print("</td>");	
		...
		for(String temp:itemList){
			...
			out.print("<td colspan=1>");
			if(oneDateSuccessMap.containsKey(temp)){
				out.print(df2.format(oneDateSuccessMap.get(temp)/oneDateMap.get(temp)));			
			}else{
				out.print("");
			}
			out.print("</td>");
