...
public class CountTribeLevel {
	...
	static public java.util.Map<Date,Map<Integer,Integer>> getAmountMap(Date fromTime,Date toTime){
		...
		try {
				rs = dbClient.execSQLQuery(
						"select ttime,level,amount from tribeinfostats where ttime>=? and ttime<=?",dbArgs);
				while(rs.next()){
					Date date = rs.getDate("ttime");
